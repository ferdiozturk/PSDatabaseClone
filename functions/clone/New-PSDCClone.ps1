            ...
            try {
                ...
                if (-not ($computer.PSobject.Properties.name -match "IsLocalhost")) {
                    if ($($env:ComputerName).toLower() -eq $($computer.ComputerName).toLower() ) {
                        ...
                        Write-PSFMessage -Message "$computer.IsLocalhost" -Level Verbose
                    }
                    else
                    {
            ...
            foreach ($db in $Database) {
                ...
                if ($PSCmdlet.ShouldProcess($ParentVhd, "Setting up clone variables")) {
                    if (-not $CloneName) {
                        ...
                        $CloneName = "$($parentVhdFile)_clone"#$parentVhdFile
                        $mountDirectory = "$($parentVhdFile)_clone_$($random)"
                    }
                    elseif ($CloneName) {
                ...
                if ($computer.IsLocalhost) {
                    if (Test-Path -Path "$Destination\$CloneName.vhdx" -Credential $DestinationCredential) {
                        Stop-PSFFunction -Message "Clone VHD $CloneName already exists" -Target $accessPath -Continue
                    }
                }
                else {
                ...
                if ($PSCmdlet.ShouldProcess($ParentVhd, "Creating VHD clone")) {
                    try {
                        Write-PSFMessage -Message "Creating VHD clone from $ParentVhd" -Level Verbose
                        ...
                        Stop-PSFFunction -Message "Could not create VHD clone" -Target $vhd -Continue -ErrorRecord $_
                    }
                }

                # Mount the vhd
                if ($PSCmdlet.ShouldProcess("$Destination\$CloneName.vhdx", "Mounting VHD clone")) {
                    try {
                        Write-PSFMessage -Message "Mounting VHD clone" -Level Verbose
                ...
                try {
                    ...
                    if ($computer.IsLocalhost) 
					{
						...
						# foreach ($dp in $(Get-Partition -Disk $disk).DiskPath)
						# {
							# $aclObject = [io.directory]::GetAccessControl("$dp")
							# $aclObject.AddAccessRule($accessRule)
							# [io.directory]::SetAccessControl("$dp", $aclObject)
						# }
                    ...
                    Stop-PSFFunction -Message "Couldn't set privileges on access path directory" -ErrorRecord $_ -Target $accessPath -Continue
                }
