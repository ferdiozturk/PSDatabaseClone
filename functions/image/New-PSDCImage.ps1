    ...
    process {
        ...
        foreach ($db in $DatabaseCollection) {
            ...
            try {
				...
				###BEGIN
				$disk = Get-Disk | Where-Object {$_.Location -eq "$accessPath.vhdx"}
...
###END
                ...
                # if ($PSCmdlet.ShouldProcess($accessPath, "Adding access path '$accessPath' to mounted disk")) {
					...
					# {
						...
						# {
							...
							# Write-PSFMessage -Message "DEBUG: $accessPath $($disk.Number) $($partition[1].PartitionNumber)" -Level Verbose
						# }
					# }
                # }
            }
            ...
            $imageDataFolder = "$accessPath\Data"
            $imageLogFolder = "$accessPath\Log"
			...
			{
				Stop-PSFFunction -Message "Couldn't set privileges on access path" -ErrorRecord $_ -Target $accessPath -Continue
			}
            ...
            if ($PSCmdlet.ShouldProcess($vhdPath, "Dismounting the vhd")) {
                ...
                try {
					...
					if ($ImageNetworkPath) 
					{
						# Check if computer is local
						if ($computer.IsLocalhost) {
							#Remove the PartitionAccessPath
							$null = Remove-PartitionAccessPath -DiskNumber $disk.Number -PartitionNumber $partition[1].PartitionNumber -AccessPath $accessPath -ErrorAction SilentlyContinue
					...
					else
					{
						If ($ImageLocalPath) 
						{
							...
							# Remove the access path
							$null = Remove-Item -Path $accessPath -Force -Recurse
						}
					}
                }
                catch {
			...
			else
			{
				if ($ImageLocalPath)
				{
					$imageLocation = "$($ImageLocalPath)\$imageName.vhdx"
				}
			}
            $sizeMB = $dbSizeMB
